...
if [ ! -x "$PYTHON_BIN" ]; then PYTHON_BIN="${MPF_PYTHON:-python}"; fi
if [ -z "$MPF_BIN" ]; then
...
EXPECTED_VERSION= DECISION_JSON= DECISION_SHA= EXECUTION_JSON= EXECUTION_SHA= ROLLBACK_JSON= ROLLBACK_SHA= ARTIFACT_JSON= ARTIFACT_SHA= SOURCE_EVIDENCE_JSON= SOURCE_EVIDENCE_SHA= OUT_DIR= OPERATOR= REASON=
EXECUTE=false
...
while [ "$#" -gt 0 ]; do
 case "$1" in
  ...
  --source-evidence-json) SOURCE_EVIDENCE_JSON="$2"; shift 2;; --source-evidence-json-sha256) SOURCE_EVIDENCE_SHA="$2"; shift 2;;
...
if { [ -n "$SOURCE_EVIDENCE_JSON" ] && [ -z "$SOURCE_EVIDENCE_SHA" ]; } || { [ -z "$SOURCE_EVIDENCE_JSON" ] && [ -n "$SOURCE_EVIDENCE_SHA" ]; }; then
 echo '--source-evidence-json and --source-evidence-json-sha256 must be provided together' >&2; exit 2
fi
mkdir -p "$OUT_DIR"
...
if [ -n "$SOURCE_EVIDENCE_JSON" ]; then check_sha "$SOURCE_EVIDENCE_JSON" "$SOURCE_EVIDENCE_SHA"; fi
...
"$PYTHON_BIN" - <<'PY' "$EXECUTION_OUT" "$OUT_DIR/manifest.json" "$EXPECTED_VERSION" "$EXECUTE" "$COLLECT_POST_EVIDENCE"
import json
import sys
from pathlib import Path

execution_path, manifest_path, expected_version, execute_requested, collect_requested = sys.argv[1:]
execute = execute_requested == "true"
manifest = {
    "component": "phase11e_limited_activation_execute_helper",
    "expected_version": expected_version,
    "execute_requested": execute,
    "collect_post_evidence_requested": collect_requested == "true",
...
if execute:
    ...
    required = {
        ...
        "db_mutation_performed": bool,
        "activation_executed": bool,
        "final_decision": str,
    }
    for key, expected_type in required.items():
        if key not in execution or not isinstance(execution[key], expected_type):
            raise ValueError(f"invalid or missing execution evidence field: {key}")
    manifest.update({
        "mutation_performed_by_helper": execution["mutation_performed"],
        "db_mutation_performed_by_helper": execution["db_mutation_performed"],
        "activation_executed_by_helper": execution["activation_executed"],
        "execution_final_decision": execution["final_decision"],
    })
...
if [ "$COLLECT_POST_EVIDENCE" = true ]; then
 ...
 POST_SOURCE_ARGS=()
 if [ -n "$SOURCE_EVIDENCE_JSON" ]; then POST_SOURCE_ARGS=(--source-evidence-json "$SOURCE_EVIDENCE_JSON" --source-evidence-json-sha256 "$SOURCE_EVIDENCE_SHA"); fi
 "$MPF_BIN" production phase11e-limited-activation-post-evidence-collect --expected-version "$EXPECTED_VERSION" --activation-execution-json "$EXECUTION_OUT" --activation-execution-json-sha256 "$(cat "$OUT_DIR/limited-activation-execution.sha256")" --artifact-gate-json "$ARTIFACT_JSON" --artifact-gate-json-sha256 "$ARTIFACT_SHA" "${POST_SOURCE_ARGS[@]}" --operator "$OPERATOR" --reason "$REASON" --operator-confirmed --i-understand-post-evidence-only --i-understand-no-db-mutation --i-understand-no-firewall-apply --i-understand-no-production-traffic-expansion --i-understand-no-miner-traffic-expansion --i-understand-no-abuse-automation --out-json "$POST_OUT" --output json
