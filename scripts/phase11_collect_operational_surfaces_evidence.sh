...
MPF_BIN="${MPF_BIN:-mpf}"
mkdir -p "$OUT_DIR/restart-autostart-proof"

run_txt() { "$@" > "$1" 2> "$1.stderr" || echo $? > "$1.rc"; }
run_json() { local out="$1"; shift; "$@" > "$out"; python3 -m json.tool "$out" >/dev/null; }
...
h=r.get('resolved_ipv4') or r.get('target_host'); p=r.get('target_port')
if r.get('status')!='ok' or r.get('blockers') or not h or not p or r.get('backend_public_exposure') is True: raise SystemExit(1)
print(f'{h}:{p}')
PY
)"
...
run_json "$OUT_DIR/restart-autostart-proof/proof-report.json" "$MPF_BIN" production restart-autostart-proof --evidence-dir "$OUT_DIR" --output json
run_json "$OUT_DIR/production-customer-lifecycle-execution-readiness.json" "$MPF_BIN" production production-customer-lifecycle-execution-readiness --evidence-dir "$OUT_DIR" --expected-backend-target "$EXPECTED_BACKEND_TARGET" --output json
run_json "$OUT_DIR/phase11-operational-completion-gap-inventory.json" "$MPF_BIN" production phase11-operational-completion-gap-inventory --evidence-dir "$OUT_DIR" --output json
"$MPF_BIN" db status > "$OUT_DIR/db-status.txt" 2>&1 || true
"$MPF_BIN" lanes > "$OUT_DIR/lanes.txt" 2>&1 || true
"$MPF_BIN" customer list > "$OUT_DIR/customer-list.txt" 2>&1 || true
if "$MPF_BIN" abuse status --output json > "$OUT_DIR/abuse-status.json" 2> "$OUT_DIR/abuse-status.stderr"; then python3 -m json.tool "$OUT_DIR/abuse-status.json" >/dev/null; else "$MPF_BIN" abuse status > "$OUT_DIR/abuse-status.txt" 2>&1 || true; fi
python3 - "$OUT_DIR" <<'PY'
...
manifest={'expected_version':(out/'expected-version.txt').read_text().strip(),'expected_backend_target':(out/'expected-backend-target.txt').read_text().strip(),'files':sorted(str(p.relative_to(out)) for p in out.rglob('*') if p.is_file())}
