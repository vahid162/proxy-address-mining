...
case "${MODE}" in
  --plan)
    exec "${MPF_BIN}" production controlled-artifact-refresh-package --mode plan "$@"
    ;;
  --package)
    exec "${MPF_BIN}" production controlled-artifact-refresh-package --mode package "$@"
    ;;
  --execute-preflight)
    exec "${MPF_BIN}" production controlled-artifact-refresh-package --mode execute-preflight "$@"
    ;;
  --execute)
    ...
    exec "${MPF_BIN}" production controlled-artifact-refresh-package --mode execute "$@"
    ;;
  --verify)
    exec "${MPF_BIN}" production controlled-artifact-refresh-package --mode verify "$@"
    ;;
  --rollback-test)
    exec "${MPF_BIN}" production controlled-artifact-refresh-package --mode rollback-test "$@"
    ;;
  -h|--help)
    usage
    ;;
