...
OUT_DIR=""; EXPECTED_VERSION=""; VIS=""; VIS_SHA=""; SRC=""; SRC_SHA=""; AG=""; AG_SHA=""; ABUSE=""; ABUSE_SHA=""; RESTART=""; RESTART_SHA=""; PRE=""; PRE_SHA=""
while [[ $# -gt 0 ]]; do case "$1" in
  ...
  --abuse-readiness-json) ABUSE="$2";shift 2;; --abuse-readiness-json-sha256) ABUSE_SHA="$2";shift 2;;
  --restart-readiness-json) RESTART="$2";shift 2;; --restart-readiness-json-sha256) RESTART_SHA="$2";shift 2;;
  --limited-acceptance-precheck-json) PRE="$2";shift 2;; --limited-acceptance-precheck-json-sha256) PRE_SHA="$2";shift 2;;
  *) echo "bad arg $1"; exit 2;; esac; done
for v in OUT_DIR EXPECTED_VERSION VIS VIS_SHA SRC SRC_SHA AG AG_SHA ABUSE ABUSE_SHA RESTART RESTART_SHA PRE PRE_SHA; do [[ -n "${!v}" ]] || { echo "missing required: $v"; exit 2; }; done
mkdir -p "$OUT_DIR"
mpf production phase11e-limited-activation-decision --expected-version "$EXPECTED_VERSION" --visibility-bundle-json "$VIS" --visibility-bundle-json-sha256 "$VIS_SHA" --source-evidence-json "$SRC" --source-evidence-json-sha256 "$SRC_SHA" --abuse-readiness-json "$ABUSE" --abuse-readiness-json-sha256 "$ABUSE_SHA" --restart-readiness-json "$RESTART" --restart-readiness-json-sha256 "$RESTART_SHA" --limited-acceptance-precheck-json "$PRE" --limited-acceptance-precheck-json-sha256 "$PRE_SHA" --artifact-gate-json "$AG" --artifact-gate-json-sha256 "$AG_SHA" --operator phase11e-helper --reason package-only --operator-confirmed --i-understand-decision-only --i-understand-no-activation-performed --i-understand-no-db-mutation --i-understand-no-firewall-apply --i-understand-no-production-traffic --i-understand-no-miner-traffic --i-understand-no-abuse-automation --i-understand-phase11-not-accepted --out-json "$OUT_DIR/limited-activation-decision.json" --output json >/dev/null
...
"$PYTHON_BIN" - <<'PY' "$OUT_DIR" "$EXPECTED_VERSION" "$VIS" "$VIS_SHA" "$SRC" "$SRC_SHA" "$AG" "$AG_SHA"
