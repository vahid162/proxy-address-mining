...
grep -q 'current_accepted_phase: Phase 5 — Customer CRUD in DB Only accepted on farm5' docs/PHASE_STATUS.md || fail 'docs/PHASE_STATUS.md does not show accepted Phase 5 DB-only customer CRUD gate'
grep -q 'current_working_phase: Phase 6 — Firewall Planner' docs/PHASE_STATUS.md || fail 'docs/PHASE_STATUS.md does not show Phase 6 as current working phase'
...
printf '%s\n' "$phase_status_output"
[[ "$phase_status_output" == *'current_accepted_phase: Phase 5 — Customer CRUD in DB Only accepted on farm5'* ]] || fail 'mpf phase-status is not aligned with accepted Phase 5 gate'
[[ "$phase_status_output" == *'current_working_phase: Phase 6 — Firewall Planner'* ]] || fail 'mpf phase-status is not aligned with Phase 6'
...
echo 'OK: current Phase 5 accepted / Phase 6 working safety gate passed. Production customer traffic remains disabled.'
