...
The accepted boundary remains Phase 11 controlled_cli_limited BTC operation. The current working gate is Phase 11 operational completion — Full CLI Production Operations. Phase 12 — Worker Policy Enforcement is blocked until operational completion acceptance. UI and Telegram remain later phases. Unrestricted production expansion and unrestricted miner expansion remain closed. Version 0.1.250 implements a source-backed controlled two-customer artifact renderer and production adapter vertical slice: `read_only_reapply_foundation_implemented=true`, `desired_artifact_semantics_complete=true`, `production_execution_available=true`, and `live_ready_package_available=false`. Public production execute is available only behind explicit package, hash, environment, host, phase, drift, lock, backup, PostgreSQL metadata, rollback, and verification gates; farm5 sync is deferred for read-only package/evidence collection first. The current next implementation step is `sync_and_collect_controlled_artifact_reapply_package_evidence_on_farm5`; restart/autostart proof remains `missing_or_partial`, Full CLI Production Operations remains unaccepted, and Phase 12/worker enforcement/UI/Telegram/timer-daemon automation remain blocked.
