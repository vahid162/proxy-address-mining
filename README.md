...
0.1.293 Phase 11 operational completion note: the official matrix is now 10 items. Generic real-customer activation is required as item 9 before item 10 final acceptance. This PR does not accept Full CLI Production Operations; production_traffic and customer_onboarding_allowed remain controlled_cli_limited, and Phase 12/worker/UI/Telegram/timers/daemons remain closed.



0.1.278 controlled lifecycle execution evidence note: adds an exact-scope DB-only production customer lifecycle execution package/preflight/execute/verify path for limited-btc-001 / btc / 20101, proving audit_event_path_availability and backup_restore_point_requirement with a reversible backup artifact and restore point. This does not accept Full CLI Production Operations, backup_restore_drill, Phase 12, worker enforcement, UI, Telegram, or cli_production gates.

0.1.276 restart/autostart strict JSON evidence note: fixes the Phase 11 restart/autostart proof helper so official JSON artifacts remain strict machine-readable JSON, passes the supplied evidence directory through post-cleanup summary and gap inventory, fails closed on malformed JSON evidence, and adds a read-only production customer lifecycle execution readiness package. Phase 11 operational completion remains not accepted; production_traffic and customer_onboarding_allowed remain controlled_cli_limited; Phase 12, worker enforcement, UI, and Telegram remain closed.
