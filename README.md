...
Latest recorded farm5 sync evidence is 0.1.123. Phase 8 Abuse 1h Core is accepted as evidence/readiness only on farm5 and is not production activation. Current target is Phase 9 report-only readiness package after 0.1.123 sync/test. Production traffic, firewall apply, iptables-restore, abuse automation runner, customer NAT/customer firewall rules, production DB execution, hard/soft block automation, pause automation, UI, and Telegram remain disabled.
