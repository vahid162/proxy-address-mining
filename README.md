...
0.1.245 farm5 post-reboot evidence found a restart/autostart persistence gap: `mpf-v2raya-socks-bridge` was unhealthy/exited and known controlled Phase 11 firewall artifacts were absent after reboot while `unknown_mpf_artifacts` remained `[]` and public backend exposure remained false. Version 0.1.247 keeps `restart_autostart_proof=missing_or_partial`, adds controlled repair plan/package/evidence tooling, and points the operator next step to `run_restart_autostart_persistence_fix_on_farm5`; it does not start a new phase, open Phase 12, or accept Full CLI Production Operations.
