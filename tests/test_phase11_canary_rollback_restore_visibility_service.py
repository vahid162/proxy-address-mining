...
def test_ready_with_concrete_artifact(monkeypatch):
    ...
    r = build_phase11_canary_rollback_restore_visibility_report(_cfg(), customer_key="canary-btc-001", lane="btc", port=20001, expected_version="0.1.215", farm5_baseline_version="0.1.168", collect_live=True)
...
def test_blocked_wrong_scope(monkeypatch):
    _patch_live(monkeypatch, canary_nat_rule_present=True, mpf_nat_pre_exists=True, prerouting_hook_present=True, canary_nat_target="172.18.0.3:60010")
    r = build_phase11_canary_rollback_restore_visibility_report(_cfg(), customer_key="wrong", lane="btc", port=20001, expected_version="0.1.215", farm5_baseline_version="0.1.168", collect_live=True)
    assert r["final_decision"] == "BLOCKED"
