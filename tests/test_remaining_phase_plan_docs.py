...
def test_current_position_updates() -> None:
    ...
    assert "- GitHub main repository version before this PR is 0.1.148." in current
    assert "- Repository version after this PR is 0.1.149." in current
    ...
    assert "- next target after this PR is farm5 sync/test evidence for Phase 11D execution gate package." in current
    assert "- Phase 11D execution gate is implemented as non-authorizing in this PR." in current
