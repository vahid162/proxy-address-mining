...
def test_current_position_updates() -> None:
    ...
    assert "- Next target after this PR is Phase 11C controlled activation harness." in current
    ...
    assert "- This PR records Phase 11A/11B farm5 sync/test evidence and keeps all runtime gates closed." in current
...
def test_phase11c_not_implemented_and_gates_closed() -> None:
    t = _read("docs/REMAINING_PHASE_PLAN.md")
    assert "Phase 11C controlled activation harness is not implemented or authorized by this PR." in t
