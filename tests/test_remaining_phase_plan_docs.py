...
def test_remaining_plan_finite_path_includes_execution_and_next_steps():
    ...
    assert "12. Phase 8 controlled worker pre-acceptance — current target in 0.1.118" in t
...
def test_remaining_plan_finite_path_numbering_and_order():
    ...
    assert finite.index("12. Phase 8 controlled worker pre-acceptance — current target in 0.1.118") < finite.index("13. Phase 8 farm5 batched sync/evidence — next")
