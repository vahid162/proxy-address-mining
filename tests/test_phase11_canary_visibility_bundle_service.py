...
def test_missing_customer_visibility(monkeypatch):
    ...
    r = build_phase11_canary_visibility_bundle_report(_cfg(), customer_key="canary-btc-001", lane="btc", port=20001, expected_version="0.1.176", farm5_baseline_version="0.1.168")
...
def test_usage_true_without_ref_is_missing(monkeypatch):
    ...
    r = build_phase11_canary_visibility_bundle_report(_cfg(), customer_key="canary-btc-001", lane="btc", port=20001, expected_version="0.1.176", farm5_baseline_version="0.1.168", evidence=ev)
...
def test_wrong_backend_target_scope_blocked(monkeypatch):
    ...
    r = build_phase11_canary_visibility_bundle_report(_cfg(), customer_key="canary-btc-001", lane="btc", port=20001, expected_version="0.1.176", farm5_baseline_version="0.1.168", collect_live=True, evidence=ev)
    assert "visibility_evidence_scope_mismatch" in r["blockers"]
    assert r["visibility"]["usage_counters_visibility"]["status"] == "MISSING"
