...
def test_blockers_and_validation_cases() -> None:
    ...
    assert phase11_manual_canary_execution_gate_service.build_phase11_manual_canary_execution_gate_report(ManualCanaryExecutionGateRequest(require_phase11d_package_evidence=False))["final_decision"] == "BLOCKED"
    ...
    assert phase11_manual_canary_execution_gate_service.build_phase11_manual_canary_execution_gate_report(ManualCanaryExecutionGateRequest(port=60010))["final_decision"] == "BLOCKED"
...
def test_cli_command_json_and_service_path(monkeypatch) -> None:
    ...
    assert json.loads(result.output)["component"] == "phase11_manual_canary_execution_gate"

    called = {"ok": False}

    def _fake(_request: ManualCanaryExecutionGateRequest) -> dict[str, object]:
        called["ok"] = True
...
def test_docs_and_plan_alignment() -> None:
    ...
    assert "latest recorded farm5 sync evidence is 0.1.147" in rem
    assert "next target after this PR is farm5 sync/test evidence for Phase 11D execution gate package" in rem
    assert "Phase 11D actual execution remains not authorized" in rem
