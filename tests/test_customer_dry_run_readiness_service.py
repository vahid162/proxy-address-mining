...
@pytest.mark.parametrize("deleted_at", ["", "   "])
def test_csv_empty_deleted_at_is_not_deleted(monkeypatch, deleted_at):
    monkeypatch.setattr(svc, "query_database_params", lambda *a, **k: DBQueryResult(True, [_customer(deleted_at=deleted_at)], "OK"))
    disable = svc.dry_run_disable_customer(_cfg(), CustomerDisableRequest(customer_key="limited-btc-001"))
    update = svc.dry_run_update_customer(_cfg(), CustomerUpdateRequest(customer_key="limited-btc-001", status="expired"))
    assert disable.ok and disable.message == "DRY_RUN_OK"
    assert update.ok and update.message == "DRY_RUN_OK"


def test_csv_non_empty_deleted_at_is_deleted(monkeypatch):
    monkeypatch.setattr(svc, "query_database_params", lambda *a, **k: DBQueryResult(True, [_customer(deleted_at="2026-06-17 00:00:00+00")], "OK"))
    res = svc.dry_run_disable_customer(_cfg(), CustomerDisableRequest(customer_key="limited-btc-001"))
    assert res.ok is False and res.message == "deleted customer cannot be mutated"


@pytest.mark.parametrize("enabled", ["t", "true", "1", True])
def test_csv_lane_enabled_truthy_values_pass(monkeypatch, enabled):
    calls = []
    ...
    def fake(cfg, sql, params=()):
        ...
        if len(calls) == 1:
            return DBQueryResult(True, [_customer()], "OK")
        return DBQueryResult(True, [{"id": 1, "enabled": enabled}], "OK")
    monkeypatch.setattr(svc, "query_database_params", fake)
    res = svc.dry_run_update_customer(_cfg(), CustomerUpdateRequest(customer_key="limited-btc-001", lane="btc"))
    assert res.ok and res.message == "DRY_RUN_OK"


@pytest.mark.parametrize("enabled", ["f", "false", "0", "", False])
def test_csv_lane_enabled_falsey_values_fail(monkeypatch, enabled):
    calls = []
