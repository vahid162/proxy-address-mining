...
def test_real_farm5_known_artifacts_pass():
    r=build_phase11_current_controlled_artifact_gate_report(iptables_save_text=REAL_BASE,phase_status_text=PHASE)
...
def test_real_unknown_chain_blocked():
    ...
    r=build_phase11_current_controlled_artifact_gate_report(iptables_save_text=t,phase_status_text=PHASE)
...
def test_ipv6_blocked():
    r=build_phase11_current_controlled_artifact_gate_report(iptables_save_text=REAL_BASE,ip6tables_save_text=':MPFC_20001 - [0:0]',phase_status_text=PHASE)
    assert r['final_decision']=='BLOCKED_UNKNOWN_MPF_ARTIFACTS'

def test_wrong_expected_version_blocked_phase_mismatch():
    r=build_phase11_current_controlled_artifact_gate_report(iptables_save_text=REAL_BASE,phase_status_text=PHASE,expected_version='0.1.208')
