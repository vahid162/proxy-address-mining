...
def test_version_docs_are_0253_after_bump():
    assert __version__ == "0.1.285"
    assert Path("VERSION").read_text().strip() == "0.1.285"
    assert 'version = "0.1.285"' in Path("pyproject.toml").read_text()
    assert "0.1.285" in Path("CHANGELOG.md").read_text()
...
def test_source_0252_bundle_verifies_under_0254_runtime_and_package_helpers(tmp_path, monkeypatch):
    ...
    assert verifier["repository_version"] == "0.1.285"
