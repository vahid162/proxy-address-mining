...
def test_phase_status_gate_alignment_and_safety_lines() -> None:
    ...
    assert result.exit_code == 0
    assert "current_accepted_phase: Phase 8 — Abuse 1h Core accepted on farm5" in result.stdout
    assert "current_working_phase: Phase 9 — Check / Report / Diagnostics planning/readiness" in result.stdout
