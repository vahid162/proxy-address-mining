...
def test_missing_source_reports_missing(monkeypatch):
    ...
    r = build_phase11_canary_reject_counters_visibility_report(_cfg(), customer_key="canary-btc-001", lane="btc", port=20001, expected_version="0.1.197", farm5_baseline_version="0.1.168", collect_live=True)
...
def test_mutation_flags_false(monkeypatch):
    monkeypatch.setattr("mpf.services.customer_read_service.list_customer_status", lambda *a, **k: customer_read_service.CustomerList(ok=True, message="ok", customers=[_active()]))
    monkeypatch.setattr("mpf.services.phase11_live_canary_evidence_collector_service.build_phase11_live_canary_evidence_collector_report", lambda *a, **k: _live_ok())
    r = build_phase11_canary_reject_counters_visibility_report(_cfg(), customer_key="canary-btc-001", lane="btc", port=20001, expected_version="0.1.197", farm5_baseline_version="0.1.168", collect_live=False)
