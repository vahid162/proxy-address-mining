...
def test_gap_inventory_cli_returns_fail_closed_json() -> None:
    ...
    assert report["repository_version"] == "0.1.262"
    assert report["final_decision"] == "PHASE11_FULL_CLI_PRODUCTION_OPERATIONS_REQUIRED"
    assert report["phase12_start_allowed"] is False
...
def test_gap_inventory_cli_accepts_packet_path_evidence_dir(monkeypatch, tmp_path) -> None:
    ...
    def fake(*, packet_path_evidence_dir=None):
        seen["path"] = packet_path_evidence_dir
        return {"component": "phase11_operational_completion_gap_inventory", "mutation_performed": False}

    monkeypatch.setattr(cli.phase11_operational_completion_gap_inventory_service, "run_phase11_operational_completion_gap_inventory_report", fake)
    ...
    assert json.loads(result.output)["mutation_performed"] is False
