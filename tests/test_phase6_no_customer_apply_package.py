...
def test_version_consistency():
    assert Path('VERSION').read_text().strip()=='0.1.91'
    assert tomllib.loads(Path('pyproject.toml').read_text())['project']['version']=='0.1.91'
    assert __version__=='0.1.91'
    r=CliRunner().invoke(app,['--version'])
    assert '0.1.91' in r.output
