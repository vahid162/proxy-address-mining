...
def test_remaining_plan_targets_updated() -> None:
    ...
    assert "GitHub main repository version before this PR is 0.1.133" in current
    assert "Repository version after this PR is 0.1.134" in current
    assert "latest recorded farm5 sync evidence is 0.1.133" in current
