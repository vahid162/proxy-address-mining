...
def test_remaining_phase_plan_active_wording() -> None:
    ...
    assert "GitHub main repository version before this PR is 0.1.106" in text
    assert "Repository version after this PR is 0.1.107" in text
    assert "latest recorded farm5 sync evidence is 0.1.104" in text
    assert "Phase 7 current target is Phase 7 read-only reports/doctor package" in text
    assert "Next target after this PR is farm5 batched sync evidence for 0.1.107" in text
    assert "operator should perform one batched offline sync for PR #113 + #114 + #115 together" in text
