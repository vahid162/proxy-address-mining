...
def _run(tmp: Path, monkeypatch, patch_db: bool = True, **overrides):
    ...
    kw = dict(
        expected_version="0.1.212", post_apply_evidence_json=post, post_apply_evidence_json_sha256=_sha(post),
...
def test_cli_smoke_json_human_and_hash_options(tmp_path, monkeypatch):
    ...
    base = [
        "production", "single-customer-runtime-probe-diagnostics", "--expected-version", "0.1.212", "--post-apply-evidence-json", str(post),
