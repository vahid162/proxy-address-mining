...
def test_final_acceptance_report_fails_closed_when_gate_missing(tmp_path: Path) -> None:
    repo = tmp_path
    (repo / 'docs/history').mkdir(parents=True, exist_ok=True)
    (repo / 'docs/history/PHASE_STATUS_LEGACY_0.1.302.md').write_text('current_accepted_phase: Phase 10 — Session / Worker / Policy / Share Timeline accepted on farm5\n', encoding='utf-8')
