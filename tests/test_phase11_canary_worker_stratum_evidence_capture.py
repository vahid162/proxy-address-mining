...
def test_cli_collect_visibility_propagates_runtime_evidence(monkeypatch, tmp_path):
    ...
    res = runner.invoke(app, ["production","canary-acceptance-review","--expected-version","0.1.185","--farm5-baseline-version","0.1.168","--collect-visibility","--visibility-json",str(usage),"--visibility-json",str(sess),"--visibility-json",str(reject),"--visibility-json",str(worker),"--output","json","--config","configs/mpf.example.yaml"])
