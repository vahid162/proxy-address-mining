...
def test_subscribe_failure_keeps_worker_visibility_false(monkeypatch):
    ...
    r = svc.build_phase11_canary_worker_stratum_evidence_capture_report(_cfg(), customer_key="canary-btc-001", lane="btc", port=20001, expected_version="0.1.193", farm5_baseline_version="0.1.168", connect_host="85.198.11.110", connect_port=20001, worker_name="canary-btc-001.worker-001", collect_live=False)
...
def test_worker_visibility_true_without_reference_not_lifted():
    ...
    r = build_phase11_canary_visibility_bundle_report(_cfg(), customer_key="canary-btc-001", lane="btc", port=20001, expected_version="0.1.193", farm5_baseline_version="0.1.168", evidence=ev)
...
def test_cli_collect_visibility_propagates_runtime_evidence(monkeypatch, tmp_path):
    ...
    res = runner.invoke(app, ["production","canary-acceptance-review","--expected-version","0.1.193","--farm5-baseline-version","0.1.168","--collect-visibility","--visibility-json",str(usage),"--visibility-json",str(sess),"--visibility-json",str(reject),"--visibility-json",str(worker),"--output","json","--config","configs/mpf.example.yaml"])
