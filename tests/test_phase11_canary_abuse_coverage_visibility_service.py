...
def test_abuse_coverage_visible_with_exact_scope(monkeypatch):
    ...
    r = build_phase11_canary_abuse_coverage_visibility_report(_cfg(), customer_key="canary-btc-001", lane="btc", port=20001, expected_version="0.1.197", farm5_baseline_version="0.1.168")
...
def test_abuse_coverage_blocked_wrong_baseline(monkeypatch):
    monkeypatch.setattr("mpf.services.customer_read_service.list_customer_status", lambda *a, **k: SimpleNamespace(ok=True, customers=[_canary_row()], message="ok"))
    r = build_phase11_canary_abuse_coverage_visibility_report(_cfg(), customer_key="canary-btc-001", lane="btc", port=20001, expected_version="0.1.197", farm5_baseline_version="0.1.167")
    assert r["final_decision"] == "BLOCKED"
...
def test_bundle_merge_preserves_abuse_allowlisted_source(monkeypatch):
    ...
    b = build_phase11_canary_visibility_bundle_report(_cfg(), customer_key="canary-btc-001", lane="btc", port=20001, expected_version="0.1.197", farm5_baseline_version="0.1.168", evidence=merged)
