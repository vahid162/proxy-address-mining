...
def test_version_0_1_133_consistency():
    assert Path("VERSION").read_text().strip() == "0.1.133"
    assert __version__ == "0.1.133"
    assert 'version = "0.1.133"' in Path("pyproject.toml").read_text(encoding="utf-8")
...
def test_phase10_farm5_0_1_132_evidence_doc_present():
    t = Path("docs/PHASE_10_FARM5_0_1_132_SYNC_TEST_EVIDENCE.md").read_text(encoding="utf-8")
    assert "server version after sync:\n  0.1.132" in t
    assert "768 passed in 149.45s" in t
