...
def test_version_0_1_135_consistency():
    assert Path("VERSION").read_text().strip() == "0.1.135"
    assert __version__ == "0.1.135"
    assert 'version = "0.1.135"' in Path("pyproject.toml").read_text(encoding="utf-8")
...
def test_phase10_farm5_0_1_134_evidence_doc_present():
    t = Path("docs/PHASE_10_FARM5_0_1_134_SYNC_TEST_EVIDENCE.md").read_text(encoding="utf-8")
    assert "server version after sync:\n  0.1.134" in t
    assert "776 passed in 140.42s" in t
