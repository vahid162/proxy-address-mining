...
BACKEND = {'status':'ok','resolved_ipv4':'172.18.0.2','target_host':'172.18.0.2','target_port':60010,'backend_public_exposure':False,'target_fingerprint':'fp'}
...
def fake_collect(*a, **k):
    return [], [], BACKEND.copy(), snap(True), '', []


def test_duplicate_cleanup_package_ready_exact_two(monkeypatch, tmp_path):
...
def test_duplicate_cleanup_no_duplicate_blocked(monkeypatch, tmp_path):
    monkeypatch.setattr(svc, '_collect_live_inputs', lambda *a, **k: ([], [], BACKEND.copy(), snap(False), '', []))
...
def test_duplicate_cleanup_backend_drift_blocked(monkeypatch, tmp_path):
    monkeypatch.setattr(svc, '_collect_live_inputs', lambda *a, **k: ([], [], BACKEND.copy(), snap(True, '172.18.0.9:60010'), '', []))
    report = svc.run_duplicate_nat_cleanup_plan_report(config_path=tmp_path/'missing')
    assert report['final_decision'] == svc.DUP_BLOCKED
...
def test_execute_requires_env_and_yes(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(svc.ProductionIptablesRestoreRunner, 'run', lambda self, argv, input_text=None: svc.CommandResult(0, '', ''))
