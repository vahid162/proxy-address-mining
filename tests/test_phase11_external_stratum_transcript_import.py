...
def test_valid_import(tmp_path):
    ...
    r=build_phase11_external_canary_stratum_transcript_import_report(_cfg(),customer_key='canary-btc-001',lane='btc',port=20001,expected_version='0.1.192',farm5_baseline_version='0.1.168',transcript_json=p)
...
def test_scope_fail_closed(tmp_path):
    ...
    for kwargs in [dict(customer_key='x',lane='btc',port=20001),dict(customer_key='canary-btc-001',lane='zec',port=20001),dict(customer_key='canary-btc-001',lane='btc',port=20002)]:
        r=build_phase11_external_canary_stratum_transcript_import_report(_cfg(),expected_version='0.1.192',farm5_baseline_version='0.1.168',transcript_json=p,**kwargs)
...
def test_wrong_worker_and_missing_notify(tmp_path):
    ...
    r2=build_phase11_external_canary_stratum_transcript_import_report(_cfg(),customer_key='canary-btc-001',lane='btc',port=20001,expected_version='0.1.192',farm5_baseline_version='0.1.168',transcript_json=p2)
...
def test_version_and_baseline_fail_closed(tmp_path):
    ...
    r2=build_phase11_external_canary_stratum_transcript_import_report(_cfg(),customer_key='canary-btc-001',lane='btc',port=20001,expected_version='0.1.192',farm5_baseline_version='0.1.167',transcript_json=p)
...
def test_collect_live_checks(monkeypatch,tmp_path):
    ...
    r=build_phase11_external_canary_stratum_transcript_import_report(_cfg(),customer_key='canary-btc-001',lane='btc',port=20001,expected_version='0.1.192',farm5_baseline_version='0.1.168',transcript_json=p,collect_live=True)
    assert r['final_decision']=='BLOCKED'
...
def test_end_to_end_visibility_acceptance(monkeypatch,tmp_path):
    ...
    b=build_phase11_canary_visibility_bundle_report(_cfg(),customer_key='canary-btc-001',lane='btc',port=20001,expected_version='0.1.192',farm5_baseline_version='0.1.168',evidence=ev)
    assert 'unique_workers_visibility' not in b['missing_visibility_primitives']
    acc=build_phase11_canary_acceptance_review_report(_cfg(),customer_key='canary-btc-001',lane='btc',port=20001,expected_version='0.1.192',farm5_baseline_version='0.1.168',evidence=Phase11CanaryAcceptanceEvidence.from_dict({'canary_customer_db_visible':True,'usage_visibility_ok':True,'reject_visibility_ok':True,'session_visibility_ok':True,'unique_ip_visibility_ok':True,'worker_visibility_ok':True,'stratum_subscribe_ok':True,'stratum_authorize_ok':True,'stratum_set_difficulty_seen':True,'stratum_notify_seen':True}))
