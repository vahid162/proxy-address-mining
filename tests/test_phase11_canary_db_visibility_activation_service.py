...
def test_plan_create_no_rows(monkeypatch):
    ...
    r = build_phase11_canary_db_visibility_activation_report(_cfg(), Phase11CanaryDbVisibilityActivationRequest(expected_version="0.1.187"))
...
def test_execute_without_confirms_blocked(monkeypatch):
    monkeypatch.setattr("mpf.services.customer_read_service.list_customer_status", lambda *a, **k: customer_read_service.CustomerList(ok=True, message="ok", customers=[]))
    req = Phase11CanaryDbVisibilityActivationRequest(requested_action="execute", expected_version="0.1.187")
...
def test_plan_non_mutating(monkeypatch):
    monkeypatch.setattr("mpf.services.customer_read_service.list_customer_status", lambda *a, **k: customer_read_service.CustomerList(ok=True, message="ok", customers=[]))
    r = build_phase11_canary_db_visibility_activation_report(_cfg(), Phase11CanaryDbVisibilityActivationRequest(expected_version="0.1.187", requested_action="plan"))
    assert r["mutation_performed"] is False
    assert r["db_mutation_performed"] is False
