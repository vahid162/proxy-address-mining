...
def readyish_package():
    ...
    snapshot_hash = _text_sha("*filter\nCOMMIT\n")
    structure_hash = _firewall_structure_sha("*filter\nCOMMIT\n")
    pkg["iptables_save_sha256"] = snapshot_hash
    pkg["ip6tables_save_sha256"] = snapshot_hash
    pkg["iptables_structure_sha256"] = structure_hash
    pkg["ip6tables_structure_sha256"] = structure_hash
...
def _executable_live_plan(pkg):
    ...
    plan["snapshot_hashes"] = {
        ...
        "iptables_structure_sha256": pkg["iptables_structure_sha256"],
        "ip6tables_structure_sha256": pkg["ip6tables_structure_sha256"],
    }
...
def _counter_snapshot(*, packets: int = 0, bytes_: int = 0, extra_rule: str = "") -> str:
    return (
        "# Generated by iptables-save\n"
        "*filter\n"
        f":INPUT ACCEPT [{packets}:{bytes_}]\n"
        ":FORWARD DROP [0:0]\n"
        ":OUTPUT ACCEPT [0:0]\n"
        f"-A INPUT -p tcp -m comment --comment \"unrelated stable rule\" -j ACCEPT [{packets}:{bytes_}]\n"
        f"{extra_rule}"
        "COMMIT\n"
        "# Completed\n"
    )
...
def _ready_counter_package(package_snapshot: str) -> dict[str, object]:
    plan = build_plan(
        lanes=lanes(),
        customers=customers(),
        backend_target=target(),
        iptables_save_text=package_snapshot,
        ip6tables_save_text=package_snapshot,
        phase_status_text=PHASE,
    )
    assert plan["final_decision"] == PACKAGE_READY
    pkg = build_package_from_plan(plan)
    pkg["__package_file_sha256"] = _canonical_sha(pkg)
    return pkg


def _execute_with_live_plan(pkg: dict[str, object], live_plan: dict[str, object]) -> dict[str, object]:
    return execute_package(
        package=pkg,
        package_sha256=str(pkg["__package_file_sha256"]),
        package_id=str(pkg["package_id"]),
        operator="op",
        reason="structure hash regression",
        execute=True,
        yes=True,
        env={"MPF_PHASE11_CONTROLLED_ARTIFACT_REAPPLY": "allow", "MPF_PHASE11_CONTROLLED_ARTIFACT_REAPPLY_EXECUTE": "allow"},
        current_hostname=str(pkg["hostname"]),
        live_plan_builder=lambda: live_plan,
        runner=ProductionRunner(),
        lock=ProductionLock(),
        backup=ProductionBackup(),
...
def test_execute_drift_allows_iptables_counter_only_changes_and_reports_raw_warning():
    package_snapshot = _counter_snapshot(packets=1, bytes_=10)
    live_snapshot = _counter_snapshot(packets=999, bytes_=8888)
    pkg = _ready_counter_package(package_snapshot)
    live_plan = build_plan(lanes=lanes(), customers=customers(), backend_target=target(), iptables_save_text=live_snapshot, ip6tables_save_text=package_snapshot, phase_status_text=PHASE)
    live_plan["iptables_save_text"] = live_snapshot
    live_plan["ip6tables_save_text"] = package_snapshot

    assert live_plan["snapshot_hashes"]["iptables_save_sha256"] != pkg["iptables_save_sha256"]
    assert live_plan["snapshot_hashes"]["iptables_structure_sha256"] == pkg["iptables_structure_sha256"]
    assert live_plan["execution_precondition_fingerprint"] == pkg["execution_precondition_fingerprint"]

    result = _execute_with_live_plan(pkg, live_plan)
    assert result["final_decision"] == "CONTROLLED_ARTIFACT_REAPPLY_EXECUTED_PENDING_FARM5_EVIDENCE_REVIEW"
    assert "iptables_snapshot_drift" not in result.get("blockers", [])
    assert "iptables_structure_snapshot_drift" not in result.get("blockers", [])
    assert result["raw_snapshot_drift_warnings"] == ["iptables_raw_snapshot_drift"]

    drift_plan = dict(live_plan, payload_sha256="drift")
    failed = _execute_with_live_plan(pkg, drift_plan)
    assert failed["final_decision"] == "FAILED_PRE_APPLY"
    assert failed["raw_snapshot_drift_warnings"] == ["iptables_raw_snapshot_drift"]
    assert "iptables_structure_snapshot_drift" not in failed["blockers"]


def test_execute_drift_blocks_real_iptables_rule_structure_change():
    pkg = _ready_counter_package(_counter_snapshot(packets=1, bytes_=10))
    live_snapshot = _counter_snapshot(extra_rule='-A INPUT -p tcp --dport 65535 -m comment --comment "real drift" -j ACCEPT\n')
    live_plan = build_plan(lanes=lanes(), customers=customers(), backend_target=target(), iptables_save_text=live_snapshot, ip6tables_save_text=_counter_snapshot(packets=1, bytes_=10), phase_status_text=PHASE)
    live_plan["iptables_save_text"] = live_snapshot
    live_plan["ip6tables_save_text"] = _counter_snapshot(packets=1, bytes_=10)

    result = _execute_with_live_plan(pkg, live_plan)
    assert result["final_decision"] == "FAILED_PRE_APPLY"
    assert "iptables_structure_snapshot_drift" in result["blockers"]


def test_ip6tables_counter_only_drift_is_stable_and_warns_on_raw_only_drift():
    package_snapshot = _counter_snapshot(packets=1, bytes_=10)
    live_v6 = _counter_snapshot(packets=1234, bytes_=5678)
    pkg = _ready_counter_package(package_snapshot)
    live_plan = build_plan(lanes=lanes(), customers=customers(), backend_target=target(), iptables_save_text=package_snapshot, ip6tables_save_text=live_v6, phase_status_text=PHASE)
    live_plan["iptables_save_text"] = package_snapshot
    live_plan["ip6tables_save_text"] = live_v6

    assert live_plan["snapshot_hashes"]["ip6tables_save_sha256"] != pkg["ip6tables_save_sha256"]
    assert live_plan["snapshot_hashes"]["ip6tables_structure_sha256"] == pkg["ip6tables_structure_sha256"]
    assert live_plan["execution_precondition_fingerprint"] == pkg["execution_precondition_fingerprint"]

    drift_plan = dict(live_plan, payload_sha256="drift")
    failed = _execute_with_live_plan(pkg, drift_plan)
    assert failed["raw_snapshot_drift_warnings"] == ["ip6tables_raw_snapshot_drift"]
    assert "ip6tables_structure_snapshot_drift" not in failed["blockers"]


def test_ip6tables_real_mpf_artifact_structure_drift_blocks():
    pkg = _ready_counter_package(_counter_snapshot(packets=1, bytes_=10))
    live_v6 = _counter_snapshot(extra_rule='-A INPUT -m comment --comment "mpf customer_v6_drift" -j ACCEPT\n')
    live_plan = build_plan(lanes=lanes(), customers=customers(), backend_target=target(), iptables_save_text=_counter_snapshot(packets=1, bytes_=10), ip6tables_save_text=live_v6, phase_status_text=PHASE)
    live_plan["iptables_save_text"] = _counter_snapshot(packets=1, bytes_=10)
    live_plan["ip6tables_save_text"] = live_v6

    result = _execute_with_live_plan(pkg, live_plan)
    assert result["final_decision"] == "FAILED_PRE_APPLY"
    assert "ip6tables_structure_snapshot_drift" in result["blockers"]
    assert "live_artifact_classification_blocked" in result["blockers"]


def test_execution_precondition_fingerprint_uses_structure_hash_not_raw_counter_hash():
    package_snapshot = _counter_snapshot(packets=1, bytes_=10)
    live_snapshot = _counter_snapshot(packets=2, bytes_=20)
    first = build_plan(lanes=lanes(), customers=customers(), backend_target=target(), iptables_save_text=package_snapshot, ip6tables_save_text=package_snapshot, phase_status_text=PHASE)
    second = build_plan(lanes=lanes(), customers=customers(), backend_target=target(), iptables_save_text=live_snapshot, ip6tables_save_text=live_snapshot, phase_status_text=PHASE)
