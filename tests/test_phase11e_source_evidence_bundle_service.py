...
def call(tmp_path, **kw):
 ...
 base=dict(expected_version='0.1.297',current_controlled_artifact_gate_sha256='abc',visibility_bundle_json=p,visibility_bundle_json_sha256=h,operator_confirmed=True,i_understand_read_only=True,i_understand_no_activation=True,i_understand_no_firewall_apply=True,i_understand_no_db_mutation=True,i_understand_no_restart=True,i_understand_no_abuse_automation=True)
 base.update(kw)
...
def full_sources():
 return dict(
  ...
  current_controlled_artifact_gate={'repository_version':'0.1.297','current_phase_gate_ok':True,'unknown_mpf_artifacts':[],'forbidden_public_runtime_exposure':False,'production_gates_remain_closed':True,'final_decision':'PASS_NO_CUSTOMER_ARTIFACTS'},
