...
def _report(monkeypatch, ev: Phase11CanaryAcceptanceEvidence, **kwargs):
    ...
    return build_phase11_canary_acceptance_review_report(
        ...
        expected_version=kwargs.get("expected_version", "0.1.184"),
...
def test_cli_collect_visibility_with_reject_evidence_advances_to_unique_workers(monkeypatch, tmp_path):
    ...
    res = runner.invoke(app,["production","canary-acceptance-review","--expected-version","0.1.184","--farm5-baseline-version","0.1.168","--collect-visibility","--visibility-json",str(usage),"--visibility-json",str(sess),"--visibility-json",str(reject),"--output","json","--config","configs/mpf.example.yaml"])
    assert res.exit_code == 0
    assert '"final_decision": "BLOCKED"' in res.stdout
