...
def test_phase10_readiness_accepted():
    ...
    assert r['latest_recorded_farm5_sync_evidence'] == '0.1.128'
...
def test_phase10_readiness_fail_closed_missing_evidence(tmp_path: Path):
    (tmp_path / 'docs').mkdir()
    phase = Path('docs/PHASE_STATUS.md').read_text(encoding='utf-8').replace('### Phase 10 farm5 0.1.128 Sync/Test Evidence', '### missing')
    ...
    assert 'farm5_0_1_128_sync_test_evidence_missing' in r['blockers']
