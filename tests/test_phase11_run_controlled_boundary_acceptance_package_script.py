...
def _run(tmp_path, decision):
    ...
    out=tmp_path/"out"; proc=subprocess.run([str(SCRIPT_PATH), "--expected-version", "0.1.233", *args, "--out-dir", str(out), "--operator", "operator", "--reason", "read-only"], env={**os.environ,"MPF_BIN":str(fake)}, check=False)
    return proc, out
