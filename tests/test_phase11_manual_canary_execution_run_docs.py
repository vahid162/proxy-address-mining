...
def test_docs_and_readme_updates() -> None:
    ...
    assert 'GitHub main repository version before this PR is 0.1.155.' in rp
    assert 'Repository version after this PR is 0.1.157.' in rp
    ...
    assert 'Next target: sync latest main to farm5, run explicit operator-approved single-canary execution once, and collect evidence; if execution blocks, implement the exact missing primitive reported as `accepted_single_canary_host_apply_primitive`.' in rp
...
def test_runbook_exists_with_exact_command() -> None:
    ...
    assert "--expected-version 0.1.157" in rb
