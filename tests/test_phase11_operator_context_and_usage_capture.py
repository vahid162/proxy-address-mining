...
def test_canary_db_visibility_execute_blocks_before_db_write_on_effective_root(monkeypatch):
    ...
    req = Phase11CanaryDbVisibilityActivationRequest(requested_action="execute", expected_version="0.1.178", operator_confirmed=True, understand_db_only_canary=True, understand_no_firewall_or_nat=True, reviewed_rollback=True, fresh_farm5_sync_confirmed=True)
...
def test_canary_db_visibility_plan_allowed_as_root(monkeypatch):
    monkeypatch.setattr("mpf.services.customer_read_service.list_customer_status", lambda *a, **k: customer_read_service.CustomerList(ok=True, message="ok", customers=[]))
    monkeypatch.setattr("mpf.services.operator_execution_context_service.build_operator_execution_context_report", lambda *a, **k: {"database_url_is_local_peer": True, "os_user": "root"})
    req = Phase11CanaryDbVisibilityActivationRequest(requested_action="plan", expected_version="0.1.178")
    r = build_phase11_canary_db_visibility_activation_report(_cfg(), req)
...
def test_usage_capture_missing_db_visibility_not_ready(monkeypatch):
    ...
    r = phase11_canary_usage_evidence_capture_service.build_phase11_canary_usage_evidence_capture_report(_cfg(), customer_key="canary-btc-001", lane="btc", port=20001, expected_version="0.1.178", farm5_baseline_version="0.1.168", collect_live=True)
...
def test_generated_usage_evidence_integration_visibility_bundle_acceptance(monkeypatch):
    monkeypatch.setattr("mpf.services.customer_read_service.list_customer_status", lambda *a, **k: customer_read_service.CustomerList(ok=True, message="ok", customers=[_active_canary()]))
    monkeypatch.setattr("mpf.services.phase11_live_canary_evidence_collector_service.build_phase11_live_canary_evidence_collector_report", lambda *a, **k: {"evidence": {"canary_nat_rule_present": True, "canary_nat_rule_count": 1, "no_extra_customer_nat_rules": True, "no_unexpected_mpf_firewall_references": True, "proxy_doctor_ok": True, "nat_counter_packets": 0, "nat_counter_bytes": 696, "canary_nat_target": "172.18.0.3:60010"}})
    cap = phase11_canary_usage_evidence_capture_service.build_phase11_canary_usage_evidence_capture_report(_cfg(), customer_key="canary-btc-001", lane="btc", port=20001, expected_version="0.1.178", farm5_baseline_version="0.1.168", collect_live=True)
    ev = cap["usage_evidence"]
    usage = build_phase11_canary_usage_visibility_report(_cfg(), customer_key="canary-btc-001", lane="btc", port=20001, expected_version="0.1.178", farm5_baseline_version="0.1.168", evidence=type("E", (), ev)() if False else None)
    ...
    usage = build_phase11_canary_usage_visibility_report(_cfg(), customer_key="canary-btc-001", lane="btc", port=20001, expected_version="0.1.178", farm5_baseline_version="0.1.168", evidence=Phase11CanaryUsageVisibilityEvidence.from_dict(ev))
    ...
    bundle = build_phase11_canary_visibility_bundle_report(_cfg(), customer_key="canary-btc-001", lane="btc", port=20001, expected_version="0.1.178", farm5_baseline_version="0.1.168", evidence=Phase11CanaryVisibilityEvidence.from_dict(ev))
    ...
    acc = build_phase11_canary_acceptance_review_report(_cfg(), customer_key="canary-btc-001", lane="btc", port=20001, expected_version="0.1.178", farm5_baseline_version="0.1.168", evidence=Phase11CanaryAcceptanceEvidence(canary_customer_db_visible=True, usage_visibility_ok=True))
