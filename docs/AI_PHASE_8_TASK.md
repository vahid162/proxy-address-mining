...
## Current Phase 8 Step — Operator-Invoked Controlled Worker Dry-Run Package

- explicit operator-invoked dry-run package only
...
- no pause automation
- no production traffic
- farm5 0.1.119 sync/test evidence is recorded
- 0.1.120 sync/test evidence is required before future farm5 dry-run evidence collection
- future dry-run execution must be separately invoked by operator
- future dry-run must remain limited, explicit, fail-closed, and non-production
- dry-run output must report skip/no-work/lock/failure/idempotency outcomes explicitly
- no silent skip is allowed
