...
Phase 11 controlled artifact reapply note (0.1.250): source-backed renderer, live classifier, read-only plan/package/verify/evidence surfaces, and production adapter wiring are implemented for exactly the two accepted BTC customers. No farm5 mutation has been performed. Next step: `prove_controlled_filter_packet_path_before_farm5_package_collection`; restart/autostart proof and Full CLI Production Operations remain unaccepted, Phase 12/worker enforcement/UI/Telegram remain blocked. See `docs/PHASE_11_CONTROLLED_ARTIFACT_REAPPLY_RUNBOOK.md`.
