...
Phase 11 controlled artifact reapply note (0.1.249): resolver and read-only plan/package/verify/evidence surfaces are implemented for exactly the two accepted BTC customers, but public production execute fails closed until real live-preflight/lock/backup/audit/rollback/verification adapters are implemented and tested. No farm5 mutation has been performed. Next step: `implement_source_backed_controlled_artifact_renderer_and_production_adapters`; restart/autostart proof and Full CLI Production Operations remain unaccepted, Phase 12/worker enforcement/UI/Telegram remain blocked. See `docs/PHASE_11_CONTROLLED_ARTIFACT_REAPPLY_RUNBOOK.md`.
...
`docs/AI_PHASE_11_OPERATIONAL_COMPLETION_TASK.md` and `docs/PHASE_11_OPERATIONAL_COMPLETION_GATE.md` are active/current for Phase 11 operational completion — Full CLI Production Operations. Version 0.1.249 records current step `implement_source_backed_controlled_artifact_renderer_and_production_adapters`: the read-only reapply foundation exists, but desired artifact semantics are incomplete, production execution is unavailable, live READY package availability is false, and restart/autostart proof plus Full CLI Production Operations remain unaccepted without source-backed evidence. `docs/AI_PHASE_11_TASK.md`, `docs/PRODUCTION_ACTIVATION_GATE.md`, and `docs/AI_SAFE_RUNTIME_FIRST.md` remain Phase 11 controlled-boundary context. `docs/AI_PHASE_10_TASK.md` is accepted Phase 10 context. `docs/AI_PHASE_8_TASK.md`, `docs/AI_PHASE_9_TASK.md`, and Phase 6 documents below are historical/reference-only and non-authorizing unless `docs/PHASE_STATUS.md` explicitly reopens them.
