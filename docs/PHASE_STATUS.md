...
Phase 11 operational completion 0.1.250 note: fixes controlled artifact reapply executor injected-environment handling so tests and callers can pass an explicit empty environment without inheriting ambient CI. No farm5 mutation was performed; artifacts are not claimed restored; next step remains `sync_and_collect_controlled_artifact_reapply_package_evidence_on_farm5`; restart/autostart proof remains `missing_or_partial`; Full CLI Production Operations remains unaccepted; Phase 12, worker enforcement, UI, Telegram, and timer/daemon automation remain blocked.

Phase 11 operational completion 0.1.249 note: controlled artifact reapply capability is implemented for exactly `canary-btc-001/btc/20001` and `limited-btc-001/btc/20101`, including source-backed backend target resolution, exact desired-state/package construction, guarded execution, verification, helper, and evidence collection. No farm5 mutation was performed by the PR, artifacts are not claimed restored, and the next step is `sync_and_collect_controlled_artifact_reapply_package_evidence_on_farm5`. `restart_autostart_proof` remains `missing_or_partial`; Full CLI Production Operations remains unaccepted; `production_traffic` and `customer_onboarding_allowed` remain `controlled_cli_limited`; Phase 12, worker enforcement, UI, Telegram, and timer/daemon automation remain blocked.
