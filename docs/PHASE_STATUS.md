...
- Phase 11 farm5 0.1.173 planning/readiness note: added read-only `mpf production canary-visibility-bundle` and optional `--collect-visibility` integration in acceptance review to classify exact-scope canary visibility primitives (customer-db/usage/reject/session/IP/worker/abuse/check/rollback) as PRESENT/MISSING/BLOCKED without opening any runtime gate; Current State gate values remain unchanged and Phase 11 remains not accepted.
