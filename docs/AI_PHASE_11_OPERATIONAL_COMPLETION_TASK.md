...
## 0.1.294 AI task update — generic real-customer activation

The next runtime-first work is to collect/review farm5 evidence for the new generic activation path: package, preflight, apply, verify, transcript/runtime evidence, first-connect DB evidence where applicable, and abuse coverage. Existing Windows synthetic Stratum evidence for `limited-btc-001:20101` is compatibility evidence only; it does not close Phase 11. Do not mark Full CLI Production Operations accepted until item 10 final acceptance is reviewed last. Do not open Phase 12, worker enforcement, UI, Telegram, timers, daemons, public backend/API, or unrestricted production.
