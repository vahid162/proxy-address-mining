# AI Coding Rules Compatibility Redirect

Status: Compatibility redirect

This active file no longer carries dynamic gate snapshots, version-specific release notes, or duplicate runtime authorization.

Use these current authorities instead:

- [`AGENTS.md`](../AGENTS.md) for AI task routing, including the Runtime-forward PR rule.
- [`docs/PHASE_STATUS.md`](PHASE_STATUS.md) for current state, phase, gates, and runtime authorization.
- [`docs/GUIDELINES.md`](GUIDELINES.md) for engineering and AI-safe implementation rules.
- [`docs/SAFETY.md`](SAFETY.md) for safety constraints.
- [`docs/ARCHITECTURE.md`](ARCHITECTURE.md) for architecture boundaries.
- [`docs/AI_SAFE_RUNTIME_FIRST.md`](AI_SAFE_RUNTIME_FIRST.md) when the active gate names runtime-first work.
- Relevant domain documents for task-specific rules.

Compatibility anchors for older validators and service readiness checks are non-authorizing labels only: Phase 8 dry-run evaluator stop condition; Phase 8 evidence/reporting contract; Phase 8 DB-only transition readiness stop condition; Phase 8 DB-only execution stop condition; Phase 8 runtime/worker readiness stop condition; Phase 8 Controlled Worker Pre-Acceptance Stop Condition; Why / What / How to test / Version / Risk + Rollback; Historical pre-final-acceptance Phase 11 planning-readiness stop condition (non-authorizing reference only).

Historical content from this file is preserved as non-authorizing context in [`docs/history/AI_CODING_RULES_LEGACY_0.1.301.md`](history/AI_CODING_RULES_LEGACY_0.1.301.md).
