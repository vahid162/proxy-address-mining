...
0.1.251 controlled artifact reapply note: source-backed rendering, strict classification, production lock/backup/audit/runner wiring, and post-apply verification gates are implemented; default plan/package/evidence commands remain read-only, and controlled execute still requires explicit package review plus environment/operator gates. No farm5 mutation was performed by the PR.
