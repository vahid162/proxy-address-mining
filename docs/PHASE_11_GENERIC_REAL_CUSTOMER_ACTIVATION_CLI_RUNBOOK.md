...
Version target after sync: `0.1.298`.
...
## 0.1.298 current gate recognition note

Version 0.1.298 teaches the current phase safety gate to recognize the official `vahid-btc-real-60046` / port `60046` generic activation artifacts produced by the controlled package/apply path. Real public backend `60010` exposure remains fail-closed. For the already-applied `60046` state, keep the controlled order: rerun the current phase safety gate, rerun generic activation verify, use the existing transcript evidence only after both are READY/PASS, run first-connect DB evidence only after verify is READY, rerun abuse coverage, rerun readiness, then stop before final acceptance. This runbook explicitly does not accept Phase 11 or Full CLI Production Operations.
...
```bash
export EVIDENCE_DIR=/tmp/phase11-generic-activation-0.1.298-vahid-btc-real-60046-$(date -u +%Y%m%dT%H%M%SZ)
