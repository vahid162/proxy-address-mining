...
0.1.292 closes pre-final Phase 11 operational usability/evidence gaps by adding operator-friendly `mpf customer show <customer_key>`, clarifying usage/report/check runtime evidence availability, normalizing firewall diagnostic expected-backend-target handling, and aligning controls readiness contract output. This PR does not accept Full CLI Production Operations, does not change production_traffic/customer_onboarding_allowed to cli_production, and does not open Phase 12, worker enforcement, UI, Telegram, timers, daemons, firewall apply, conntrack, Docker, systemd, or DB/customer mutation. next_required_step must pass production_generic_real_customer_activation before final_phase11_operational_completion_acceptance.
0.1.293 note: Phase 11 operational completion now uses a 10-item official matrix. Item 9 is `production_generic_real_customer_activation`; item 10 is final acceptance, which remains the last gate and is the only step that may set `production_traffic=cli_production` and `customer_onboarding_allowed=cli_production`. Full CLI Production Operations is not accepted; production/customer gates remain controlled_cli_limited; Phase 12, worker enforcement, UI, Telegram, timers, and daemons remain closed.


0.1.290 fixes 0.1.289 farm5 post-sync pytest regression by keeping Phase 11 gap inventory evidence-driven for controls readiness. No runtime mutation, no final acceptance, and no Phase 12/worker/UI/Telegram opening.
0.1.289 defines read-only Phase 11 customer block control-intent preflight, completes production_controls_pause_block_expire readiness, advances the next required step to backup_restore_drill when prior evidence is ready, and performs no runtime mutation, no final acceptance, and no Phase 12 opening.
...
0.1.275 post-cleanup restart/autostart and controlled artifact persistence evidence note: farm5 0.1.274 evidence completed controlled duplicate NAT cleanup for canary-btc-001:20001 and limited-btc-001:20101 with backend target 172.18.0.2:60010 healthy, backend_public_exposure=false, unknown_mpf_artifacts=[], duplicate_nat_redirect_count=0, duplicate_controlled_artifact_count=0, forbidden_public_runtime_exposure=false, production gates still closed, and next_required_step=collect_restart_autostart_and_controlled_artifact_persistence_evidence. This release adds read-only post-cleanup restart/autostart plus controlled artifact persistence evidence tooling and expected_backend_target-aware nested artifact gate propagation. Phase 11 operational completion remains not accepted; production_traffic and customer_onboarding_allowed remain controlled_cli_limited; Phase 12, worker enforcement, UI, and Telegram remain closed.

0.1.274 controlled duplicate NAT cleanup note: adds the official operator-facing controlled duplicate NAT cleanup plan/package/execute-preflight/execute/verify/rollback-contract path and post-cleanup readiness summary for the exact Phase 11 canary-btc-001:20001 and limited-btc-001:20101 duplicate NAT redirects. The path resolves and passes expected_backend_target into the current controlled artifact gate, remains operator-gated, writes backups/evidence/manifests during execute, and does not restart Docker/systemd, flush conntrack, mutate DB/customer/policy/abuse state, enable Phase 12, worker enforcement, UI, Telegram, unrestricted production, or accept Phase 11 operational completion. production_traffic and customer_onboarding_allowed remain controlled_cli_limited.


0.1.272 post-refresh graph verification note: farm5 0.1.271 successfully enabled the controlled stale-artifact refresh and the runtime state is healthy with the corrected post-DNAT graph, unknown_mpf_artifacts=[], forbidden_public_runtime_exposure=false, production gates closed, and the stale 0.1.269 graph absent. Version 0.1.272 fixes post-refresh verification to trust the current controlled artifact gate plus strict post-DNAT semantic checks, fixes BLOCKED/FAILED CLI exit codes, writes complete execute evidence files with consistent backup naming, detects controlled duplicate MPF NAT redirects, adds an operator-gated duplicate NAT cleanup package contract, and prevents generic reapply planning from emitting the legacy pre-DNAT INPUT/public-port payload after the corrected graph is present. Phase 11 operational completion is not accepted; production_traffic and customer_onboarding_allowed remain controlled_cli_limited; Phase 12, worker enforcement, UI, and Telegram remain closed.
0.1.271 refresh command sequence update: after syncing 0.1.271 to farm5, operators must run the read-only controlled refresh plan/package, run execute-preflight, send the package/preflight evidence for review, execute the controlled refresh only if the operator-gated READY evidence remains valid, and then collect post-apply current-controlled-artifact-gate, refresh verify, and runtime evidence. This only enables the next controlled refresh attempt; farm5 remains controlled_cli_limited, Full CLI Production Operations is not accepted, and Phase 12, worker enforcement, UI, and Telegram remain closed.

0.1.271 controlled stale-artifact refresh note: 0.1.270 rendered corrected post-DNAT graph semantics, but farm5 live-ready remained blocked because the stale 0.1.269 live artifact graph was still present and append-only reapply could leave the old MPF_GUARD before corrected DNAT accounting/customers hooks. This release adds an operator-gated controlled refresh/rollback-first package path that recognizes only the exact known stale 0.1.269 controlled graph for canary-btc-001:20001 and limited-btc-001:20101, blocks mixed or unknown MPF artifacts, requires backup/lock/restore-test/verification contracts, and renders targeted MPF-owned replacement payloads. farm5 remains controlled_cli_limited; Full CLI Production Operations is not accepted; Phase 12, worker enforcement, UI, and Telegram remain closed. Next required step: sync 0.1.271 to farm5, run read-only refresh package preflight, then execute the controlled refresh only if operator-gated READY evidence remains valid.

0.1.270 controlled post-DNAT graph fix note: farm5 0.1.269 controlled reapply succeeded with controlled artifacts present, backend target 172.18.0.2:60010 healthy/local-only, backend_public_exposure=false, unknown_mpf_artifacts=[], and no rollback required; the external farm4→farm5 85.198.11.110:20101 probe then proved valid DNAT customer traffic was rejected by MPF_GUARD before MPF_ACCT_IN/MPF_CUSTOMERS. This release fixes verified Docker-user post-DNAT semantics with conntrack original-destination matching for 20001/20101 plus any-mode MPFC_<port> -> MPFO_<port> policy dispatch, while direct external backend 60010 remains rejected. farm5 remains controlled_cli_limited; Full CLI Production Operations is not accepted; Phase 12, worker enforcement, UI, and Telegram remain closed. Next required step: sync 0.1.270 to farm5, run read-only/package/preflight, then run the controlled refresh or rollback-first apply workflow.
...
### 0.1.298 — Phase 11 current gate recognizes official generic activation artifacts

- Recognizes official Phase 11 generic real-customer activation artifacts for `vahid-btc-real-60046` / port `60046` in the current controlled artifact gate.
- Keeps unrelated or malformed unknown MPF artifacts fail-closed.
- Keeps backend public exposure forbidden.
- Does not run first-connect DB.
- Does not accept Full CLI Production Operations.
- Does not set `production_traffic=cli_production`.
- Does not set `customer_onboarding_allowed=cli_production`.
- Does not open Phase 12, worker enforcement, UI, or Telegram.
- Next farm5 step after sync: rerun current phase safety gate; rerun generic-activation-verify using a fresh 0.1.298 snapshot; if both are READY/PASS, run first-connect DB evidence.
...
0.1.262 controlled reapply live-ready execute note: guarded execute now revalidates the reviewed live-ready packet-path binding at execution time before any `iptables-restore --test` or apply. It preserves package/file/id/operator/reason/--execute/--yes/environment gates, CI blocking, production-only adapters, PostgreSQL audit metadata, real host lock, real backup adapter, `iptables-restore --test` before apply, exact two-customer scope, unknown-artifact/public-exposure fail-closed behavior, and rollback_required behavior for failed apply/post-apply verification. No PR-development farm5 mutation is claimed; Phase 12, worker enforcement, UI, Telegram, timers, daemons, Docker/systemd action, conntrack flush, customer/policy/abuse mutation, and unrestricted production remain blocked.
0.1.261 controlled reapply guarded helper note: hardens the controlled artifact reapply shell helper with non-mutating execute-preflight support, script-level --yes for execute, immediate execution-gate preflight before execute, evidence report capture, and manifest writing. This PR performs no controlled artifact reapply execution. restart_autostart_proof remains missing_or_partial; full_cli_production_operations remains missing_or_partial; production_traffic remains controlled_cli_limited; customer_onboarding_allowed remains controlled_cli_limited; phase12_start_allowed remains no; worker_enforcement_allowed, ui_allowed, and telegram_allowed remain no.
0.1.260 controlled reapply execution gate preflight note: adds a read-only controlled reapply execution gate preflight surface only. It does not execute controlled artifact reapply. It does not authorize iptables-restore, firewall mutation, DB mutation, Docker/systemd/conntrack mutation, customer mutation, abuse mutation, Phase 12, worker enforcement, UI, Telegram, timers, daemons, or unrestricted production. Current gates remain `production_traffic=controlled_cli_limited`, `customer_onboarding_allowed=controlled_cli_limited`, `controlled_artifact_execute_available=false`, `iptables_restore_invocation_allowed=false`, and all mutation flags false.
...
### 2026-06-17 — 0.1.288 Phase 11 post-sync validation and firewall evidence handoff stabilization

0.1.288 stabilizes post-sync Phase 11 validation and resolves collector firewall completion evidence wrapper/nested handoff after 0.1.287. No runtime mutation, no final acceptance, no Phase 12. production_traffic and customer_onboarding_allowed remain controlled_cli_limited; worker_enforcement_allowed, ui_allowed, telegram_allowed, and phase12_start_allowed remain closed.
...
### 0.1.281 Runtime-first verifier note

0.1.281 separates copied historical lifecycle source-evidence mutation flags from current collector-run mutation summaries, aligns read-only controlled-artifact reapply diagnostics with target-aware artifact-gate evidence, and adds a fail-closed production firewall apply/verify/rollback completion evidence contract. This does not mark Full CLI Production Operations accepted, does not mark `production_firewall_apply_verify_rollback` ready without complete controlled execution evidence, does not change `production_traffic` or `customer_onboarding_allowed`, and does not open Phase 12. The next required step remains `production_firewall_apply_verify_rollback`.
...
## 0.1.278 Phase 11 controlled lifecycle execution evidence

Version 0.1.278 adds a controlled, exact-scope, DB-only lifecycle execution evidence path for `limited-btc-001` / `btc` / `20101`. The operation packages and preflights expected state, creates a JSON restore artifact plus `backups` and `restore_points` rows, writes `events` and `audit_log` evidence, and updates only safe lifecycle metadata. Valid verifier evidence may close `audit_event_path_availability` and `backup_restore_point_requirement` for Full CLI Production Operations item #2 only. It does not mark `backup_restore_drill`, `full_cli_production_operations`, `production_traffic=cli_production`, `customer_onboarding_allowed=cli_production`, Phase 12, worker enforcement, UI, or Telegram ready.
...
## 0.1.294 Phase 11 generic activation execution note

Version 0.1.294 adds the generic real-customer activation execution and first-connect evidence path for Phase 11 operational completion item 9. The new generic path is operator-gated and fail-closed: real host execution requires package hash, customer key, public port, pre-apply snapshot, rollback artifact, operator lock id, and the explicit `MPF_PHASE11_GENERIC_ACTIVATION_APPLY=1` environment gate; it runs `iptables-restore --test --noflush` before `iptables-restore --noflush` and reports mutation flags only after successful apply. Verification can consume an `iptables-save` text snapshot through the generic converter, and transcript import plus first-connect DB evidence are available as controlled evidence paths.

The existing Windows synthetic Stratum evidence proves only the already-controlled `limited-btc-001:20101` path. It does not close Phase 11 and does not prove generic activation for a new customer. New generic customer activation still requires package, preflight, apply, verify, runtime/transcript, first-connect DB evidence when `activation_mode=first_connect`, and abuse coverage evidence before final acceptance can be reviewed. Full CLI Production Operations remains unaccepted: `production_traffic` and `customer_onboarding_allowed` remain `controlled_cli_limited`. Final acceptance remains official item 10 and must remain last. Phase 12, worker enforcement, UI, Telegram, timers, daemons, public backend, public API, and unrestricted production remain closed.
