...
def _classify_forwarder_pool_seen(messages: list[dict[str, object]], source_ip: str | None, source_port: int | None, forwarder_logs: str | None) -> bool:
    if not forwarder_logs:
        return False
    has_pool = 'bitcoin.viabtc.io:3333' in forwarder_logs
    has_backend = '172.18.0.3:60010' in forwarder_logs
    has_src = bool(source_ip and source_ip in forwarder_logs)
    has_sport = bool(source_port and f':{source_port}' in forwarder_logs)
    return bool(has_pool and has_backend and (has_src or has_sport))
...
def build_phase11_external_canary_stratum_transcript_import_report(config: MPFConfig, *, customer_key: str, lane: str, port: int, expected_version: str, farm5_baseline_version: str, transcript_json: Path, collect_live: bool = False) -> dict[str, object]:
    ...
    norm = {
        ...
        'worker_name': worker_name, 'source_ip': source_ip, 'messages': messages,
    }
    ...
    if collect_live:
        ...
        fw_logs = live.get('source_artifacts', {}).get('forwarder_logs', '') if isinstance(live.get('source_artifacts'), dict) else ''
        forwarder_pool_seen = _classify_forwarder_pool_seen(messages, source_ip if isinstance(source_ip,str) else None, source_port if isinstance(source_port,int) else None, fw_logs)
        bridge_loopback_seen = bool(live_ev.bridge_loopback_seen)

    worker_visibility_ok = bool(scope_ok and subscribe_ok and authorize_ok and transcript_hash and worker_reference)
    evidence = Phase11CanaryVisibilityEvidence(
        ...
        worker_reference=worker_reference if worker_visibility_ok else None,
    ...
    return {
        'component':'phase11_external_canary_stratum_transcript_import', 'expected_version':expected_version, 'repository_version':__version__,
        'farm5_baseline_version':farm5_baseline_version, 'customer_key':customer_key, 'lane':lane, 'public_port':port,
        'connect_host':raw_obj.get('connect_host'), 'connect_port':connect_port, 'transcript_hash':transcript_hash,
        'mutation_performed':False,'db_mutation_performed':False,'firewall_mutation_performed':False,'nat_mutation_performed':False,'conntrack_mutation_performed':False,'docker_mutation_performed':False,
        'production_traffic_enabled':False,'phase11_accepted':False,'limited_onboarding_allowed':False,'no_onboarding_authorized':True,
        'worker_evidence':{'worker_visibility_ok':worker_visibility_ok,'worker_reference':evidence.worker_reference,'unique_worker_count':1 if worker_visibility_ok else 0,'unique_workers':[worker_name] if worker_visibility_ok else [],
        'stratum_subscribe_ok':subscribe_ok,'stratum_authorize_ok':authorize_ok,'stratum_set_difficulty_seen':set_diff_seen,'stratum_notify_seen':notify_seen,
        'forwarder_pool_seen':forwarder_pool_seen,'bridge_loopback_seen':bridge_loopback_seen,'evidence_source':evidence.evidence_source,'evidence_reference':evidence.evidence_reference},
        'generated_evidence':asdict(evidence),'blockers':sorted(set(blockers)),'warnings':sorted(set(warnings)),
