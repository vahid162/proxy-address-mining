...
class Phase11SingleCanaryHostApplyExecutor:
    ...
    def execute(self, report: dict[str, object], payload: str) -> dict[str, object]:
        ...
        if "-N MPF_NAT_PRE" in payload or "-A PREROUTING" in payload:
        ...
        if sum("--dport 20001" in l for l in payload.splitlines()) != 1 or sum("127.0.0.1:60010" in l for l in payload.splitlines()) != 1:
            return {"status": "blocked", "error": "single_canary_restore_payload_not_apply_safe", "missing_primitive": "accepted_apply_safe_single_canary_payload"}
        ...
        canary_count = sum("canary-btc-001" in l and "--dport 20001" in l and "127.0.0.1:60010" in l for l in lines)
