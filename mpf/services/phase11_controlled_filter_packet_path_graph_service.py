...
def build_packet_path_graph(*, collection_id: str, hostname: str, backend: dict[str, Any], network: dict[str, Any], topology: dict[str, Any], parsed_ipv4: dict[str, Any]) -> PacketPathGraph:
    ...
    nodes = [
        ...
        PacketPathNode("docker_accept", "docker_accept", "Docker/filter ACCEPT"),
    ...
    if route_class == "forwarded":
        edges.append(PacketPathEdge("post_dnat_route_decision", "forward", "routes_to", {"route_evidence_ref": "host-network-topology.json:route_get_backend", "route_device": bridge}))
    elif route_class == "local_input":
        edges.append(PacketPathEdge("post_dnat_route_decision", "local_input", "routes_to", {"route_evidence_ref": "host-network-topology.json:route_get_backend"}))
    else:
        edges.append(PacketPathEdge("post_dnat_route_decision", "forward", "not_reachable_from", {"route_evidence_ref": "host-network-topology.json:route_get_backend"}))
    for rule in parsed_ipv4.get("rules", []) if isinstance(parsed_ipv4.get("rules"), list) else []:
        ...
        dst = _node_for_chain(target) if target else "other_filter_chain"
        edges.append(PacketPathEdge(src, dst, "jumps_to" if rule.get("jump_kind") == "jump" else "routes_to" if rule.get("jump_kind") == "goto" else "precedes", {"table": "filter", "source_chain": rule.get("chain"), "rule_index": rule.get("rule_index"), "rule_hash": rule.get("rule_hash")}))
    edges.append(PacketPathEdge("backend_bridge", "backend_container", "reachable_from", {"docker_network_evidence_ref": "sanitized-docker-network.json", "backend_evidence_ref": "sanitized-backend-target.json"}))
...
def _node_for_chain(chain: str) -> str:
    ...
    if chain.startswith("MPF"):
        return "future_mpf_filter_entry"
    return "other_filter_chain"
...
def classify_route(*, topology: dict[str, Any], backend: dict[str, Any], network: dict[str, Any]) -> str:
    ...
    if isinstance(route, list) and len(route) == 1 and isinstance(route[0], dict):
        item = route[0]
        if str(item.get("type", "unicast")) in {"unreachable", "blackhole", "prohibit"}:
            return "unresolved"
