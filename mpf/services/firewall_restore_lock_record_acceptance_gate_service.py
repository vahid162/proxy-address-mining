...
def build_restore_lock_record_acceptance_gate_report(cfg: MPFConfig, repo_root: Path | None = None) -> dict[str, object]:
    ...
    if not phase_status.exists():
        ...
        if "System clock synchronized: no" in text:
            blockers.append("current server warning still presents System clock synchronized: no as current unresolved status")
        ...
        if "194.225.150.25" not in text:
            blockers.append("farm5 time sync evidence does not include NTP source 194.225.150.25")
    ...
    return {
        ...
        "gate_status": "ACCEPTANCE_PREREQUISITES_READY",
        ...
        "future_restore_point_gate_prereq_ready": read_only_snapshot_evidence_present and restore_lock_record_gate_server_sync_evidence_present,
        "future_lock_gate_prereq_ready": restore_lock_record_readiness_evidence_present and restore_lock_record_gate_proposal_present,
        "future_db_apply_record_gate_prereq_ready": restore_lock_record_gate_server_sync_evidence_present and farm5_time_sync_resolved,
        ...
        "migration_allowed": False, "migration_executed": False,
        ...
        "next_required_gate": "operator-approved controlled restore point + lock + DB apply record execution gate with fresh farm5 evidence",
        "blockers": blockers,
        "warnings": warnings,
        "errors": errors,
    }
