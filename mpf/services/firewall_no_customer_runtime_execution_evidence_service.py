...
from mpf.services import firewall_apply_gate_readiness_service, firewall_no_customer_runtime_execution_approval_service
...
def build_no_customer_runtime_execution_evidence_report(cfg: MPFConfig, repo_root: Path | None = None) -> dict[str, object]:
    ...
    farm5_0_1_94_sync_evidence_present = False
    ...
    if not phase_status.exists():
        ...
        farm5_0_1_94_sync_evidence_present = "farm5 synced to 0.1.94" in text and "mpf --version: 0.1.94" in text
    ...
    if not farm5_0_1_94_sync_evidence_present:
        blockers.append("farm5 0.1.94 sync evidence missing")

    runtime_approval = firewall_no_customer_runtime_execution_approval_service.build_no_customer_runtime_execution_approval_report(cfg, repo_root=root)
    runtime_approval_report_present = bool(runtime_approval)
    ...
    checks = [
        (runtime_approval_report_present, "runtime approval report missing"),
        (runtime_approval_report_blocked, "runtime approval report not BLOCKED"),
        (runtime_approval_execution_disallowed, "runtime approval execution_allowed true"),
        (apply_gate_readiness_present, "apply-gate-readiness missing"),
        (apply_gate_readiness_blocked, "apply-gate-readiness not BLOCKED"),
        (gate_review_json_evidence_present, "gate-review JSON evidence missing"),
        (gate_review_json_blocked, "gate-review JSON not BLOCKED"),
        (gate_review_json_non_applyable, "gate-review JSON not non-applyable"),
        (gate_review_json_live_apply_disallowed, "gate-review JSON live_apply_allowed not false"),
    ...
    checklist_rows = [
        ...
        ("farm5_0_1_94_sync_evidence_present", farm5_0_1_94_sync_evidence_present, "farm5 sync 0.1.94 evidence recorded"),
        ("runtime_approval_report_present", runtime_approval_report_present, "runtime approval report exists"),
        ("runtime_approval_report_blocked", runtime_approval_report_blocked, "runtime approval final_decision=BLOCKED"),
        ("runtime_approval_execution_disallowed", runtime_approval_execution_disallowed, "runtime approval execution_allowed=false"),
        ("apply_gate_readiness_blocked", apply_gate_readiness_blocked, "apply-gate-readiness final_decision=BLOCKED"),
        ("gate_review_json_blocked", gate_review_json_blocked, "gate-review final_decision=BLOCKED"),
        ("gate_review_json_non_applyable", gate_review_json_non_applyable, "gate-review applyable=false"),
        ("gate_review_json_live_apply_disallowed", gate_review_json_live_apply_disallowed, "gate-review live_apply_allowed=false"),
        ...
        ("no_customer_nat", True, "customer NAT remains disallowed"),
        ("no_customer_firewall_rules", True, "customer firewall rules remain disallowed"),
        ...
        ("explicit_verify_evidence_required", True, "verify evidence required in future PR"),
        ("explicit_rollback_evidence_required", True, "rollback evidence required in future PR"),
    ]

    return {
        "component": "firewall_no_customer_runtime_execution_evidence",
        "phase": "Phase 6 — Firewall Planner",
        "gate_type": "controlled_no_customer_runtime_execution_evidence",
        "final_decision": "BLOCKED",
        "authorization_status": "CONTROLLED_NO_CUSTOMER_RUNTIME_EVIDENCE_DEFINED_NOT_EXECUTED",
        "evidence_status": "READY_FOR_OPERATOR_SERVER_EVIDENCE_COLLECTION",
        ...
        "fresh_farm5_runtime_execution_evidence_required": True,
        "separate_runtime_execution_pr_required": True,
        "apply_decision": "BLOCKED",
        "verify_decision": "BLOCKED",
        "rollback_decision": "BLOCKED",
        "current_state_preserved": current_state_preserved,
        "farm5_0_1_94_sync_evidence_present": farm5_0_1_94_sync_evidence_present,
        "runtime_approval_report_present": runtime_approval_report_present,
        "runtime_approval_report_blocked": runtime_approval_report_blocked,
        "runtime_approval_execution_disallowed": runtime_approval_execution_disallowed,
        "apply_gate_readiness_present": apply_gate_readiness_present,
        "apply_gate_readiness_blocked": apply_gate_readiness_blocked,
        "gate_review_json_evidence_present": gate_review_json_evidence_present,
        "gate_review_json_blocked": gate_review_json_blocked,
        "gate_review_json_non_applyable": gate_review_json_non_applyable,
        "gate_review_json_live_apply_disallowed": gate_review_json_live_apply_disallowed,
        "apply_mode_plan_only": apply_mode_plan_only,
        "runtime_activation_disabled": runtime_activation_disabled,
        "phase_gate_firewall_apply_disallowed": phase_gate_firewall_apply_disallowed,
        "phase_gate_production_traffic_none": phase_gate_production_traffic_none,
        ...
        "no_usage_automation": True,
        "no_abuse_automation": True,
        ...
        "controlled_no_customer_runtime_execution_evidence_checklist": [
            {"item": item, "status": "PASS" if ok else "BLOCKED", "evidence": evidence}
            for item, ok, evidence in checklist_rows
        ],
        "future_farm5_no_customer_runtime_execution_evidence_runbook": {
            "steps": [
                {"item": step, "executable": False, "executed": False}
                for step in [
                    "confirm GitHub main and farm5 are synced", "confirm mpf --version", "run mpf config validate", "run mpf doctor", "run mpf db status",
                    "run mpf proxy doctor", "run mpf firewall no-customer-runtime-execution-approval --output json",
                    "run mpf firewall no-customer-runtime-execution-evidence --output json", "run mpf firewall apply-gate-readiness --output json",
                    "run mpf firewall gate-review --source config-only --output json", "confirm final_decision remains BLOCKED",
                    "confirm execution_allowed remains false", "confirm applyable remains false", "confirm live_apply_allowed remains false",
                    "confirm no customer NAT/rules exist", "confirm production traffic remains none",
                ]
            ]
        },
        "blockers": blockers,
