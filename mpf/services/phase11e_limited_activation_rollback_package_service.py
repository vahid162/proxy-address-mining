...
def build_phase11e_limited_activation_rollback_package_report(config: MPFConfig, **k: object)->dict[str,object]:
    ...
    ready=not b
    return {"component":"phase11e_limited_activation_rollback_package","expected_version":str(k.get('expected_version',__version__)),"repository_version":__version__,"candidate_customer_key":"limited-btc-001","rollback_customer_key":"limited-btc-001","rollback_lane":"btc","rollback_public_port":20101,"rollback_backend_target":"172.18.0.3:60010","rollback_plan":["verify limited-btc-001 status","pause limited-btc-001 if needed","verify canary-btc-001 preserved","verify no unknown artifacts","verify no public backend exposure","collect post-rollback evidence"],"mutation_performed":False,"blockers":sorted(set(b)),"warnings":[],"final_decision":"PHASE11E_LIMITED_ACTIVATION_ROLLBACK_PACKAGE_READY" if ready else "BLOCKED"}
