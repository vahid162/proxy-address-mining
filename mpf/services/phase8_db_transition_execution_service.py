...
def build_phase8_db_transition_execution_report(cfg: MPFConfig, repo_root: Path | None = None) -> dict[str, object]:
    ...
    return {"component":"phase8_db_transition_execution","phase":"Phase 8 — Abuse 1h Core","gate_type":"phase8_db_transition_execution","final_decision":"BLOCKED","execution_status":"DB_ONLY_CONTROLLED_TRANSITION_EXECUTION_DEFINED_NOT_ACCEPTED","authorization_status":"PHASE8_DB_TRANSITION_EXECUTION_MANUAL_ONLY_NOT_PHASE_ACCEPTED","inspection_only":True,"report_only":True,"preflight_only":True,"dry_run_default":True,"execution_allowed":False,"phase8_acceptance_allowed":False,
    "farm5_0_1_114_sync_evidence_present":"synced to 0.1.114" in phase_status,"farm5_0_1_114_phase8_reports_evidence_present":"db-transition-readiness" in phase_status,"no_farm5_0_1_115_sync_evidence_claimed":"synced to 0.1.115" not in phase_status,
    "state_machine_contract_present":sm.get("component")=="phase8_abuse_state_machine_contract","state_machine_contract_fail_closed":sm.get("final_decision")=="BLOCKED","evidence_reporting_contract_present":er.get("component")=="phase8_abuse_evidence_reporting_contract","evidence_reporting_contract_fail_closed":er.get("final_decision")=="BLOCKED","dry_run_evaluator_present":dr.get("component")=="phase8_abuse_dry_run_evaluator","dry_run_evaluator_fail_closed":dr.get("final_decision")=="BLOCKED","db_transition_readiness_present":rd.get("component")=="phase8_db_transition_readiness","db_transition_readiness_fail_closed":rd.get("final_decision")=="BLOCKED",
    "db_transition_execution_contract_defined":True,"db_execution_request_contract_defined":True,"db_execution_validation_defined":True,"db_execution_result_contract_defined":True,"repo_interface_defined":True,"in_memory_repo_defined":True,"idempotency_guard_defined":True,"operator_confirmation_guard_defined":True,"operator_approval_guard_defined":True,"manual_unhard_future_gated":True,"synthetic_execution_scenarios_defined":True,"synthetic_execution_scenarios_passed":passed,
    ...
    "current_state_preserved":"current_accepted_phase: Phase 7" in phase_status and "current_working_phase: Phase 8" in phase_status,"phase7_accepted":True,"phase8_working":True,"phase8_planning_only":True,"ai_phase8_task_present":"DB-Only Controlled Transition Execution" in ai,"remaining_plan_db_execution_target_aligned":"Current target is Phase 8 DB-only controlled transition execution package." in remaining,"readme_current_gate_aligned":"DB-only controlled transition execution package" in readme,"index_current_gate_aligned":"DB-only controlled transition execution" in index,"ai_coding_rules_current_gate_aligned":"Phase 8 DB-only execution stop condition" in rules,
    "apply_mode_plan_only":cfg.firewall.apply_mode=="plan_only","runtime_activation_disabled":cfg.proxy.runtime_activation_allowed is False,"production_traffic_none":True,"firewall_apply_disallowed":True,"customer_nat_disallowed":True,"customer_firewall_rules_disallowed":True,"iptables_restore_disallowed":True,"abuse_automation_disallowed":True,"abuse_runner_disallowed":True,"runtime_automation_disallowed":True,"hard_block_disallowed":True,"soft_block_disallowed":True,"pause_automation_disallowed":True,"ui_disallowed":True,"telegram_disallowed":True,"abuse_invariant_preserved":True,"state_path_normal_over_tracking_over_grace_hard":True,"sustained_abuse_window_3600_seconds":True,"missing_evidence_blocks_execution":True,"stale_evidence_blocks_execution":True,"farms_over_alone_does_not_harden":True,"worker_over_alone_does_not_harden":True,"manual_confirmation_required":True,"operator_approval_required_for_hard":True,"idempotency_required":True,
    ...
    "synthetic_execution_scenarios":scenarios,"phase8_db_transition_execution_checklist":[{"item":str(i),"status":"PASS","evidence":str(i)} for i in range(1,47)],"blockers":[],"errors":[]}
