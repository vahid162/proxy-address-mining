...
def build_manual_canary_customer_proposal_report(cfg: MPFConfig, repo_root: Path | None = None) -> dict[str, object]:
    ...
    report = {
        "component": "firewall_manual_canary_customer_proposal",
        "phase": "Phase 6 — Firewall Planner",
        "gate_type": "manual_canary_customer_nat_firewall_proposal",
        "final_decision": "BLOCKED",
        "authorization_status": "MANUAL_CANARY_PROPOSAL_DEFINED_NOT_AUTHORIZED",
        ...
        "separate_canary_acceptance_pr_required": True,
        ...
        "farm5_0_1_95_sync_evidence_present": farm5_0_1_95_sync_evidence_present,
        "controlled_no_customer_runtime_evidence_present": bool(controlled),
        "controlled_no_customer_runtime_evidence_blocked": controlled.get("final_decision") == "BLOCKED",
        "controlled_no_customer_runtime_evidence_execution_disallowed": not bool(controlled.get("execution_allowed", False)),
        "runtime_approval_report_present": bool(runtime),
        "runtime_approval_report_blocked": runtime.get("final_decision") == "BLOCKED",
        "runtime_approval_execution_disallowed": not bool(runtime.get("execution_allowed", False)),
        ...
        "future_manual_canary_customer_proposal": {
            "candidate_customer_required": True,
            "current_non_deleted_customer_required_for_execution": True,
            "current_server_has_no_non_deleted_customers": True if "no non-deleted customers" in text else "unknown",
            "canary_scope": "one manually selected customer only",
            "lane_scope": "one lane only",
            "production_traffic_authorized": False,
            "customer_nat_authorized": False,
            "customer_firewall_rules_authorized": False,
            "rollback_required_before_apply": True,
            "verify_required_after_apply": True,
            "explicit_operator_approval_required": True,
            "separate_server_evidence_required": True,
        },
    }
    checklist_items = [
        "current_state_preserved","farm5_0_1_95_sync_evidence_present","controlled_no_customer_runtime_evidence_present","controlled_no_customer_runtime_evidence_blocked",
        "controlled_no_customer_runtime_evidence_execution_disallowed","runtime_approval_report_blocked","runtime_approval_execution_disallowed","apply_gate_readiness_blocked",
        "config_apply_mode_plan_only","proxy_runtime_activation_disabled","no_customer_nat_authorized","no_customer_firewall_rules_authorized","no_production_traffic",
        "no_usage_automation","no_abuse_automation","separate_canary_acceptance_pr_required","fresh_farm5_canary_evidence_required","explicit_verify_evidence_required","explicit_rollback_evidence_required",
    ]
    pass_map = {
        "config_apply_mode_plan_only": report["apply_mode_plan_only"], "proxy_runtime_activation_disabled": report["runtime_activation_disabled"], "no_customer_nat_authorized": True,
        "no_customer_firewall_rules_authorized": True, "no_production_traffic": True, "no_usage_automation": True, "no_abuse_automation": True,
        "explicit_verify_evidence_required": True, "explicit_rollback_evidence_required": True,
    }
    report["manual_canary_customer_proposal_checklist"] = [
        {"item": item, "status": "PASS" if bool(pass_map.get(item, report.get(item, False))) else "BLOCKED", "evidence": item} for item in checklist_items
    ]
